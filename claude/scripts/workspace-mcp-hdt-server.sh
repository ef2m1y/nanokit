#!/bin/bash
# workspace-mcp HTTP server lifecycle management (HDT Google account)
# Usage: workspace-mcp-hdt-server.sh [start|stop|status]
#
# Serves Google Workspace tools for the HDT account
# (n.muramatsu@hyper-digitaltwins.com) as a single shared streamable-http
# endpoint on port 8324, so that parallel Claude Code sessions do not race on
# the OAuth refresh_token (Issue workspace-mcp #506/#667).
#
# This uses PORTABLE OAuth credentials (a per-account token cache dir), which is
# independent of the HDT *Claude* account (CLAUDE_CONFIG_DIR=~/.claude-hdt).
# That is deliberate: it lets the personal Claude account reach HDT Google data
# (e.g. HDT calendar for cross-account scheduling) from any folder.
#
# Same OAuth client as the personal server (claude-google-oauth in the OS secret
# store); only the credentials dir and user email differ. See workspace-mcp-
# personal-server.sh for the sibling personal (:8322) launcher.

# Graceful exit for environments without uvx (SessionStart hook runs everywhere).
command -v uvx >/dev/null 2>&1 || exit 0

PORT="${WORKSPACE_MCP_HDT_PORT:-8324}"
HOST="localhost"
...
cmd_start() {
  ...
  if [ -n "$existing_pid" ]; then
    if health_check; then
      echo "workspace-mcp-hdt already running (PID $existing_pid)" >&2
      exit 0
    fi
  ...
  echo "[$(date '+%Y-%m-%d %H:%M:%S')] workspace-mcp-hdt starting port=$PORT email=$USER_GOOGLE_EMAIL_VALUE creds=$CREDENTIALS_DIR_VALUE" >> "$LOG_FILE"
  ...
  for i in $(seq 1 60); do
    if [ -n "$(get_pid_on_port)" ] && health_check; then
      echo "workspace-mcp-hdt started (PID $(cat "$PID_FILE"), port $PORT)" >&2
      exit 0
    fi
    sleep 0.5
  done

  echo "workspace-mcp-hdt failed to start (check $LOG_FILE)" >&2
  exit 1
}
...
cmd_stop() {
  ...
  rm -f "$PID_FILE"
  echo "workspace-mcp-hdt stopped" >&2
}
